...
func TestRemoteKvVersion(t *testing.T) {
	...
	writeDB := mdbx.NewMDBX(logger).InMem().WithTablesCfg(f).MustOpen()
	defer writeDB.Close()
	conn := bufconn.Listen(1024 * 1024)
	grpcServer := grpc.NewServer()
	go func() {
		remote.RegisterKVServer(grpcServer, remotedbserver.NewKvServer(ctx, writeDB, nil))
...
func setupDatabases(t *testing.T, logger log.Logger, f mdbx.TableCfgFunc) (writeDBs []kv.RwDB, readDBs []kv.RwDB) {
	...
	ctx := context.Background()
	writeDBs = []kv.RwDB{
		mdbx.NewMDBX(logger).InMem().WithTablesCfg(f).MustOpen(),
		mdbx.NewMDBX(logger).InMem().WithTablesCfg(f).MustOpen(), // for remote db
	}
	...
	grpcServer := grpc.NewServer()
	f2 := func() {
		remote.RegisterKVServer(grpcServer, remotedbserver.NewKvServer(ctx, writeDBs[1], nil))
		if err := grpcServer.Serve(conn); err != nil {
			logger.Error("private RPC server fail", "err", err)
		}
	}
