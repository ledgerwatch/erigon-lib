...
func TestReadOnlyMode(t *testing.T) {
	path := t.TempDir()
	logger := log.New()
	db1 := mdbx.NewMDBX(logger).Path(path).WithTablesCfg(func(defaultBuckets kv.TableCfg) kv.TableCfg {
		return kv.TableCfg{
	...
	db2 := mdbx.NewMDBX(logger).Readonly().Path(path).WithTablesCfg(func(defaultBuckets kv.TableCfg) kv.TableCfg {
		return kv.TableCfg{
			kv.Headers: kv.TableCfgItem{},
		}
	}).MustOpen()
