...
func (opts MdbxOpts) Open() (kv.RwDB, error) {
	...
	if opts.flags&mdbx.Accede == 0 {
		if opts.label == kv.ChainDB {
			log.Info(fmt.Sprintf("[db] params: growStep=%s, mapsSize=%s, shrinkThreshold=%s, pageSize=%s, ", opts.growthStep, opts.mapSize, datasize.ByteSize(opts.shrinkThreshold), datasize.ByteSize(opts.pageSize)) +
				...
				fmt.Sprintf("roTxsLimiter=%d", opts.roTxsLimiter))
		}
