...
func (opts MdbxOpts) Open() (kv.RwDB, error) {
	...
	if opts.label == kv.ChainDB {
		err = env.SetDebug(mdbx.LogLvlExtra, mdbx.DbgDoNotChange, mdbx.LoggerDoNotChange) // temporary disable error, because it works if call it 1 time, but returns error if call it twice in same process (what often happening in tests)
		if err != nil {
	...
	if opts.flags&mdbx.Accede == 0 {
		...
		if opts.augumentLimit == 0 {
			opts.augumentLimit = MAX_AUGMENT_LIMIT // 128 * 256 * 1024 // mdbx's default 256 * 1024
		}
