...
type sortByNonce struct{ k uint64 }

func (i *sortByNonce) Less(than btree.Item) bool {
	return i.k < than.(*sortByNonce).k
}

type sortByNonce2 struct{ k uint64 }

func (i sortByNonce2) Less(than btree.Item) bool {
	return i.k < than.(sortByNonce2).k
}
...
func BenchmarkCache1(b *testing.B) {
	...
	for i := uint64(0); i < 10_000; i++ {
		for j := uint64(0); j < 4; j++ {
			c.ReplaceOrInsert(&sortByNonce{j*100_000 + i})
		}
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		c.Ascend(func(i btree.Item) bool {
			_ = i.(*sortByNonce).k
			return true
		})
	}
...
func BenchmarkCache2(b *testing.B) {
	...
	for i := uint64(0); i < 10_000; i++ {
		for j := uint64(0); j < 4; j++ {
			c.ReplaceOrInsert(sortByNonce2{j*100_000 + i})
		}
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func (e E) Less(than btree.Item) bool { return e < than.(E) }
