...
func NewPoolDB() kv.RwDB {
	return mdbx.NewMDBX(log.New()).InMem().Label(kv.TxPoolDB).WithTablesCfg(func(_ kv.TableCfg) kv.TableCfg { return kv.TxpoolTablesCfg }).MustOpen()
}
func NewDownloaderDB() kv.RwDB {
	return mdbx.NewMDBX(log.New()).InMem().Label(kv.DownloaderDB).WithTablesCfg(func(_ kv.TableCfg) kv.TableCfg { return kv.DownloaderTablesCfg }).MustOpen()
}
func NewSentryDB() kv.RwDB {
	return mdbx.NewMDBX(log.New()).InMem().Label(kv.SentryDB).WithTablesCfg(func(_ kv.TableCfg) kv.TableCfg { return kv.SentryTablesCfg }).MustOpen()
}
