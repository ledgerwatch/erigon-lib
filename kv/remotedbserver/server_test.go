...
//func (s *KvServer) Block(context.Context, *remote.BlockRequest) (*remote.BlockReply, error) {
//
//}
...
// DBViews - allows stateless grpc API on stateful "db transaction" objects.
// Transactions opened by external clients can't live too long.
// Props:
// - 1 tx per databaseViewID
// - thread-safe
// - auto-close tx after TTL
// - on close - open new tx - doesn't matter if new tx has same or different databaseViewID
// - return error if too old databaseViewID requested
type DBViews struct {
	mu  sync.RWMutex
	id  uint
...
func (s *threadSafeTx) view(f func(kv.Tx) error) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return f(s.tx)
}
func (s *threadSafeTx) rollback() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.tx.Rollback()
}

func newDBViews() *DBViews {
	return &DBViews{txs: make(map[uint64]*threadSafeTx)}
}

func (s *DBViews) rollbackAfterTTL(tx *threadSafeTx) {
	time.AfterFunc(1*time.Minute, func() {
		s.mu.Lock()
		defer s.mu.Unlock()
		delete(s.txs, tx.tx.ViewID())
		tx.rollback()
	})
}

func (s *DBViews) get(databaseViewID uint64) (*threadSafeTx, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	tx, ok := s.txs[databaseViewID]
	return tx, ok
}

func (s *DBViews) begin(ctx context.Context, requestedViewID uint64) (*threadSafeTx, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	tx, ok := s.txs[requestedViewID]
	if ok {
		return tx, nil
	}
	// open new tx and check if it has same or new viewID
	dbTx, err := s.db.BeginRo(ctx)
	if err != nil {
		return nil, err
	}
	newViewID := dbTx.ViewID()
	if newViewID == requestedViewID {
		tx = &threadSafeTx{tx: dbTx}
		s.txs[requestedViewID] = tx
		s.rollbackAfterTTL(tx)
		return tx, nil
	}

	// if it has new viewID but such tx already registered, just close current tx
	_, ok = s.txs[newViewID]
	if ok {
		dbTx.Rollback()
		return nil, err
	}
	s.txs[newViewID] = tx
	s.rollbackAfterTTL(tx)
	return nil, fmt.Errorf("requested too old requestedViewID. please retry")
}

func (s *DBViews) View(ctx context.Context, databaseViewID uint64, f func(kv.Tx) error) error {
	// fast-path over read-lock
	tx, ok := s.get(databaseViewID)
	if ok {
		return tx.view(f)
	}

	// slow-path over write-lock
	tx, err := s.begin(ctx, databaseViewID)
	if err != nil {
		return err
	}
	return tx.view(f)
