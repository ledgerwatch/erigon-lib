...
func (d *Domain) mergeFiles(valuesFiles, indexFiles, historyFiles []*filesItem, r DomainRanges, maxSpan uint64) (valuesIn, indexIn, historyIn *filesItem, err error) {
	...
	if r.values {
		...
		if valuesIn.index, err = buildIndex(valuesIn.decompressor, idxPath, d.tmpdir, count, false /* values */); err != nil {
...
func (ii *InvertedIndex) mergeFiles(files []*filesItem, startTxNum, endTxNum uint64, maxSpan uint64) (*filesItem, error) {
	...
	if comp, err = compress.NewCompressor(context.Background(), "Snapshots merge", datPath, ii.tmpdir, compress.MinPatternScore, ii.workers, log.LvlDebug); err != nil {
	...
	if outItem.index, err = buildIndex(outItem.decompressor, idxPath, ii.tmpdir, keyCount, false /* values */); err != nil {
